...
ClawBytes is the public signal-surface of the Claws / OpenClaw ecosystem — a curated Telegram channel (`@clawbytes`, channel ID `-1003850321704`) that posts four staggered "lanes" per day:
...
- **End-to-end publish with curator**: `clawbytes_threads.py publish --category ship --preview --use-curator` runs full pipeline but posts to a test Telegram chat instead of `-1003850321704`. Run before any production deploy.
