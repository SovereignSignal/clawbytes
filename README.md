...
| `PROTON_USER` / `PROTON_PASS` | Proton Mail account for the email digest (`claws_digest.py`) | ❌ |
| `DIGEST_RECIPIENT` | Recipient address for the email digest | ❌ |
| `MAINTAINER_DM_CHAT_ID` | Chat ID the supervisor DMs with daily status | ❌ |

All credentials are read from the environment (or an untracked local `CREDS.md`); none are committed to the repo.
