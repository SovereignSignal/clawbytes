...
- **Coding harnesses and agent CLIs, open or closed source** — Claude Code, Codex CLI, Cursor, Devin Desktop (ex-Windsurf), GitHub Copilot (agent/CLI modes), Google Antigravity, Kiro, Pi, OMP (Oh My Pi), Herdr, Kilo Code, Kimi Code, Mistral Vibe, Grok Build, Open Interpreter, Deep Agents, Codewhale, MiMo Code, Aider, Cline, Roo Code, Goose, OpenHands, OpenCode, Zed (agentic editing), Amp, Devin, Warp, Replit Agent, and credible new entrants. Releases, changelog moves, pricing/limit changes that hit operators, capability shifts, and post-mortems. The harness space is wide and fast-moving — bias toward covering a real move from a smaller harness over silence.
