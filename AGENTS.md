...
- **Tests:** `venv/bin/python -m pytest tests/ content-engine/tests/ -q` (no network/secrets needed).
...
- **Run the app (dry, safe):** `venv/bin/python clawbytes_threads.py collect --run-monitors --summary` fetches from ~9 live source classes over the network (~60s) and classifies into lanes — **no secrets required**. Then inspect with `… status`, `… audit`, `… yield-snapshot`, and `… preview --category <ship|watch|read|community>`. `preview`/`collect`/`audit`/`yield-snapshot` never post.
