...
def check_content_engine(env: dict[str, str], memory_dir: Path) -> Check:
    out_dir = memory_dir.parent / "content-engine-out"
    out_dir.mkdir(parents=True, exist_ok=True)
    run_env = env.copy()
    run_env["PYTHONPATH"] = str(REPO_ROOT / "content-engine" / "src")
    proc = run_cmd(
        [
            sys.executable,
            "-m",
    ...
    packet_path = out_dir / "weekly_signal_packet.json"
    sections = {}
    if packet_path.exists():
        packet = json.loads(packet_path.read_text(encoding="utf-8"))
        sections = {key: len(items) for key, items in packet.get("sections", {}).items()}
    ok = proc.returncode == 0 and bool(sections)
    detail = "weekly packet generated" if ok else "failed"
    if sections:
        detail += f" ({sum(sections.values())} items)"
    return Check(
        "content engine",
        ok,
        detail,
        {"stdout": proc.stdout[-1200:], "stderr": proc.stderr[-1200:], "sections": sections, "out_dir": str(out_dir)},
    )
...
def main() -> int:
    ...
    parser.add_argument("--content-engine", action="store_true", help="Generate content-engine weekly outputs")
    ...
    try:
        ...
        if args.network:
            ...
            if args.content_engine:
                checks.append(check_content_engine(network_env, network_memory))
            keep_dir = network_memory if args.keep_temp else None
        else:
            if args.content_engine:
                checks.append(check_content_engine(env, memory_dir))
